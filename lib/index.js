...
module.exports = function XEntity(options) {
  ...
  */
  Entity.asyncList$ = function asyncList$() {
    ...
    if (!_lodash2.default.isNumber(query.limit$)) query.limit$ = _limit;
    if (!_lodash2.default.isObject(query.sort$)) query.sort$ = { id: -1 };
    ...
    if (returnEntity) return _asyncList$(query).then(function (ents) {
      return _bluebird2.default.resolve({ rows: ents, pages: 50 });
    });
    ...
    return _asyncList$(query).then(function (ents) {
      ...
      _lodash2.default.each(ents, function (ent) {
        return rows.push(_formatEntity(ent));
      });
      return _bluebird2.default.resolve({ rows: rows, pages: 50 });
    });
  };
