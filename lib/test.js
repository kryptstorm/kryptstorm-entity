...
describe("XEntity - Basic", function () {
  ...
  it("Read many row", function (done) {
    ...
    dog.asyncList$({ fields$: returnFields }).then(function (loadedDogs) {
      ...
      (0, _chai.expect)(loadedDogs).to.be.an("object");
      (0, _chai.expect)(loadedDogs.rows).to.be.an("array");
      (0, _chai.expect)(loadedDogs.pages).to.be.an("number");
      (0, _chai.expect)(loadedDogs.rows[0]).to.be.an("object");

      // All properties has been loaded
      (0, _chai.expect)(loadedDogs.rows[0].name).to.be.exist;
      (0, _chai.expect)(loadedDogs.rows[0].createdAt).to.be.exist;
      (0, _chai.expect)(loadedDogs.rows[0].status).to.be.exist;
